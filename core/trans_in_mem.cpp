...
int trans_in_mem::get_input_vol(TRAN_SEQ seq, BTC_VOL* v, ERROR_CODE *err,int& long_btc_vol) {
	...
	*err = trans_cache->load(seq, (unsigned char *)&t);
	...
	if (t.long_btc_vol == 0) {	
		long_btc_vol = 0;
		*err = av_cache->multiload(index,t.input_num, (unsigned char *)v);
	}
	else {
		long_btc_vol = 1;
		*err = av_cache->multiload(index,t.input_num*2, (unsigned char *)v);
	}
	*err = NO_ERROR;
	return t.input_num;

}
int trans_in_mem::get_output_vol(TRAN_SEQ seq, BTC_VOL* v, ERROR_CODE *err, int& long_btc_vol) {
	if (seq >= num_trans) *err = INVALID_TRAN_SEQ;
	struct tran_info t;
	*err = trans_cache->load(seq, (unsigned char *)&t);
	...
	if (t.long_btc_vol == 0) {
		long_btc_vol = 0;
		index += t.input_num;
		*err = av_cache->multiload(index, t.output_num, (unsigned char *)v);
	}
	else {
		long_btc_vol = 1;
		index = index + t.input_num * 2;
		*err = av_cache->multiload(index, t.output_num * 2, (unsigned char *)v);
	}
	*err = NO_ERROR;
	return t.output_num;
